create function public.search_derived_memory(p_profile text,p_chat_id text,p_sender_id text,p_query text,p_trigger text default null)
...
ranked as (
select c.*, (select count(*) from words where position(w in lower(c.text))>0) + case when p_trigger is not null and c.trigger_name=p_trigger then 3 else 0 end as score
...
$$;
revoke all on function public.search_derived_memory(text,text,text,text,text) from public,anon,authenticated;
grant execute on function public.search_derived_memory(text,text,text,text,text) to service_role;
