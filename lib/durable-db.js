  ...
  const db = new Proxy(client, { get(target, key) {
    ...
    return table => {
      ...
      const execute = () => pending ||= (async () => {
        ...
        }); } catch (err) { result = { error: { message: err.message || 'rpc exception' } }; }
        if (result.error) {
          const detail = typeof result.error === 'object' && result.error?.message
            ? `: ${String(result.error.message).slice(0, 220)}`
            : '';
          const error = new Error(`Durable database step failed: ${table} ${request.operation}${detail}`);
