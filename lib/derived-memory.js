...
export function selectDerivedMemory(rows, events, access, query, trigger, budget = 6000, references = []) {
  ...
  const eligible = rows.filter(row => {
    ...
    const score = words.filter(word => haystack.includes(word)).length +
      ...
      (references.some(ref => ref.source_table === row.table && String(ref.source_id) === String(row.id)) ? 5 : 0);
...
export async function loadDerivedMemory(db, { profile, source, chatScope, telegram, query, trigger, references = [], mode = 'shadow' }) {
  ...
  try {
    const archive = typeof db.rpc === 'function' ? await db.rpc('search_derived_memory', {
      ...
      p_references: references.filter(ref => TABLES.includes(ref.source_table)).slice(0,30).map(ref => ({source_table:ref.source_table,source_id:ref.source_id}))
    }) : null;
    ...
    const records = selectDerivedMemory(rows, events, { profile, chatId: telegram.chatId, senderId: telegram.senderId }, query, trigger, 6000, references);
